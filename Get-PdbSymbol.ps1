<#
    ...
    .EXAMPLE
        ...
        $fileList = Get-ChildItem -Path 'C:\Windows\System32' -Recurse -Force -File | Where-Object { ($PSItem.Name -like '*.exe') -or ($PSItem.Name -like '*.dll') }
        Get-PdbSymbol -Path $fileList -DestinationStore 'C:\Symbols'
...
function Get-PdbSymbol {
    ...
    function Invoke-FileDownloadWithProgress($Url, $TargetFile) {
...
'@     
    ...
    foreach ($file in $Path) {
        ...
        $bytes = $reader.ReadBytes([System.Runtime.InteropServices.Marshal]::SizeOf([type][Utilities.BinaryHelper+IMAGE_DOS_HEADER]))
        $gcHandle = [System.Runtime.InteropServices.GCHandle]::Alloc($bytes, [System.Runtime.InteropServices.GCHandleType]::Pinned)
        $dosHeader = [System.Runtime.InteropServices.Marshal]::PtrToStructure($gcHandle.AddrOfPinnedObject(), [type][Utilities.BinaryHelper+IMAGE_DOS_HEADER])
        $gcHandle.Free()
        
        [void]$fileStream.Seek($dosHeader.e_lfanew, [System.IO.SeekOrigin]::Begin)
        try { [void]$reader.ReadUInt32() }
        # Not good, I know.
        catch { }
        
        $bytes = $reader.ReadBytes([System.Runtime.InteropServices.Marshal]::SizeOf([type][Utilities.BinaryHelper+IMAGE_FILE_HEADER]))
        $gcHandle = [System.Runtime.InteropServices.GCHandle]::Alloc($bytes, [System.Runtime.InteropServices.GCHandleType]::Pinned)
        $fileHeader = [System.Runtime.InteropServices.Marshal]::PtrToStructure($gcHandle.AddrOfPinnedObject(), [type][Utilities.BinaryHelper+IMAGE_FILE_HEADER])
        $gcHandle.Free()
        
        $bytes = $reader.ReadBytes([System.Runtime.InteropServices.Marshal]::SizeOf([type][Utilities.BinaryHelper+IMAGE_OPTIONAL_HEADER64]))
        $gcHandle = [System.Runtime.InteropServices.GCHandle]::Alloc($bytes, [System.Runtime.InteropServices.GCHandleType]::Pinned)
        $optHeader = [System.Runtime.InteropServices.Marshal]::PtrToStructure($gcHandle.AddrOfPinnedObject(), [type][Utilities.BinaryHelper+IMAGE_OPTIONAL_HEADER64])
        $gcHandle.Free()
        ...
        $imgSecHeader = [Utilities.BinaryHelper+IMAGE_SECTION_HEADER[]]::new($fileHeader.NumberOfSections)
        
        for ($headerNo = 0; $headerNo -lt $imgSecHeader.Length; $headerNo++) {
            
            $bytes = $reader.ReadBytes([System.Runtime.InteropServices.Marshal]::SizeOf([type][Utilities.BinaryHelper+IMAGE_SECTION_HEADER]))
            $gcHandle = [System.Runtime.InteropServices.GCHandle]::Alloc($bytes, [System.Runtime.InteropServices.GCHandleType]::Pinned)
            $imgSecHeader[$headerNo] = [System.Runtime.InteropServices.Marshal]::PtrToStructure($gcHandle.AddrOfPinnedObject(), [type][Utilities.BinaryHelper+IMAGE_SECTION_HEADER])
            $gcHandle.Free()
        ...
        [void]$fileStream.Seek($offDebug, [System.IO.SeekOrigin]::Begin)
        
        while ($cbDebug -ge [System.Runtime.InteropServices.Marshal]::SizeOf([type][Utilities.BinaryHelper+IMAGE_DEBUG_DIRECTORY])) {
            if ($loopExit -eq 0) {
                $bytes = $reader.ReadBytes([System.Runtime.InteropServices.Marshal]::SizeOf([type][Utilities.BinaryHelper+IMAGE_DEBUG_DIRECTORY]))
                $gcHandle = [System.Runtime.InteropServices.GCHandle]::Alloc($bytes, [System.Runtime.InteropServices.GCHandleType]::Pinned)
                $imgDebugDir = [System.Runtime.InteropServices.Marshal]::PtrToStructure($gcHandle.AddrOfPinnedObject(), [type][Utilities.BinaryHelper+IMAGE_DEBUG_DIRECTORY])
                $gcHandle.Free()
                
                $seekPosition = $fileStream.Position
        
                if ($imgDebugDir.Type -eq 2) {
                    [void]$fileStream.Seek($imgDebugDir.PointerToRawData, [System.IO.SeekOrigin]::Begin)
                    
                    $bytes = $reader.ReadBytes([System.Runtime.InteropServices.Marshal]::SizeOf([type][Utilities.BinaryHelper+IMAGE_DEBUG_DIRECTORY_RAW]))
                    $gcHandle = [System.Runtime.InteropServices.GCHandle]::Alloc($bytes, [System.Runtime.InteropServices.GCHandleType]::Pinned)
                    $debugInfo = [System.Runtime.InteropServices.Marshal]::PtrToStructure($gcHandle.AddrOfPinnedObject(), [type][Utilities.BinaryHelper+IMAGE_DEBUG_DIRECTORY_RAW])
                    $gcHandle.Free()
            ...
            $cbDebug -= [System.Runtime.InteropServices.Marshal]::SizeOf([type][Utilities.BinaryHelper+IMAGE_DEBUG_DIRECTORY])
        }
        ...
        if (!(Test-Path -Path "$destinationPath\$pdbName" -PathType Leaf)) {
            ...
            try {
                Invoke-FileDownloadWithProgress -Url $downloadUrl -TargetFile "$destinationPath\$pdbName"
            }
            catch {
        ...
        $fileStream.Dispose()
        $processedFileCount++
        Write-Progress -Activity 'Downloading Symbols' -Status "Processed files: $processedFileCount/$($Path.Count)" -PercentComplete (($processedFileCount / $Path.Count) * 100)
    }
}