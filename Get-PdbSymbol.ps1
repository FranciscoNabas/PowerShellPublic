...
'@     
        ...
        $cacheWriter = [System.IO.File]::AppendText("$DestinationStore\.GetPdbSyProcessedCache.txt")
    
        $processedFileCount = 1
        foreach ($file in $Path) {
    
            if ($existingFilenames) {
                if ($existingFilenames.Contains([System.IO.Path]::GetFileNameWithoutExtension($file))) {
            ...
            if (!(Test-Path -Path "$destinationPath\$pdbName" -PathType Leaf)) {
                ...
                [void]$cacheWriter.WriteAsync("$downloadUrl`n")
                try {
